...
class resendOTP_api extends REST_Controller {

    public function __construct() {

        //load user model
		parent::__construct();
       	$this->load->model("Login_model", "resendOTP");
	...
	function resendOTP_post(){
		...
		if($returnAuth == 1){
			...
			$data1 = $this->resendOTP->reSendOTP_user($Data);
			if(empty($data1)){
				...
				$data['data']['message'] = 'View User Details';
				$data['data']['response_code'] = '200';
				$data['data']['verification'] = $data1;
				$this->response($data);
			}
